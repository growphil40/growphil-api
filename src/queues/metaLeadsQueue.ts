...
// ─── Main Worker ─────────────────────────────────────────────────────────────
export const metaLeadsWorker = new Worker(
  'meta-leads',
  async (job) => {
...
const queueEvents = new QueueEvents('meta-leads', { connection: connection as any });

queueEvents.on('failed', async ({ jobId, failedReason }) => {
  try {
    const job = await metaLeadsQueue.getJob(jobId);
    if (!job) return;
...
metaLeadsWorker.on('failed', (job, err) => {
...
).catch(err => logger.error('MetaLeadsQueue', 'Failed to schedule sync-leads cron', { error: err.message }));
